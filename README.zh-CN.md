...
`notewell init --agent <claude|cursor|codex>` 或
`notewell init --guide general`，代替 `onboard`。
...
- `notewell onboard [dir]`：通过交互式引导选择目标目录、agent skills 和知识管理
  guide；使用 `--yes` 可按默认值执行。
- `notewell init --guide general [dir]`：额外生成
  `wiki/guides/knowledge-management.md`。
- `notewell init --agent claude [dir]`：额外生成 Claude 可用的 Notewell
  organize、ingest、query、lint Skills。
...
1. 把原始资料放到 `raw/` 或 `raw/inbox/`，附件建议放在 `raw/assets/`。
2. inbox 变乱时，用经过确认的 `notewell-organize` 计划整理 raw 文件。
3. ingest raw 文件到 `wiki/sources/`，再在 `wiki/` 中维护长期有用的总结、概念、
   分析、问题和 playbook。
4. 使用 wikilink 连接相关页面，例如 `[[wiki/concepts/recomposition]]`。
5. 运行 `notewell index .`。
6. 运行 `notewell lint .`。
7. Agent 需要检索上下文时，运行 `notewell search "query" .`。
8. 重要变更使用 `notewell log --type note "message" .` 记录。
...
每个被选择的 adapter 都会生成完整的 `notewell-organize`、`notewell-ingest`、
`notewell-query` 和 `notewell-lint` Skills。`notewell-organize` 负责在 ingest
前规划 raw 文件整理，并要求移动或重命名前必须得到用户确认。`notewell-query`
包含硬规则：Search the vault before answering。
